...
proto.handle = function handle(req, res, out) {
  ...
  // setup next layer
  // 这里的赋值有什么意义吗. 可以直接在处理函数里面, 使用 req.next 取得 nextHandler???
...
// 可见, 对于 Layer 的判断, 其实还是以 Path 为主的. 
proto.route = function route(path) {
  ...
  var layer = new Layer(path, {
    ...
    route.dispatch.bind(route)); // 以 Route 为主体的 Layer, 它的主要的函数处理, 就是调用 route 的 dispatch. 
...
methods.concat('all').forEach(function (method) {
  // 所有的 get, post 等 http 方法, 都在 router 的 proto 里面进行了方法的注册. 
  // 每一次这种方法的调用, 都是创建了一个 Route 中间件.
  proto[method] = function (path) {
    // 每一次 router 的调用, 都会给自己的 stack 增加一个 Layer
    var route = this.route(path)
    // The apply() method calls the specified function with a given this value, and arguments provided as an array (or an array-like object).
    // 之所以这里使用了 apply, 主要是 arguments 是一个类数组的值, 需要找一种方式进行参数值的传递. 
 ...
 * @private
 */
// Layer 首先是根据 Path 进行判断. 只有 Path 相符, 才会到这个 Layer 的处理流程中. 
// 这个 Path 有着树形关系.
