...
Route.prototype._handles_method = function _handles_method(method) {
  ...
  if (this.registeredMethods._all) {
    return true;
  }

  var name = method.toLowerCase();

  if (name === 'head' && !this.registeredMethods['head']) {
    name = 'get';
  }
  // methods 里面, 标记了这个 Route 到底处理那些 Http 方法. 
  return Boolean(this.registeredMethods[name]);
};
...
Route.prototype._options = function _options() {
  var methods = Object.keys(this.registeredMethods);

  // append automatic head
  if (this.registeredMethods.get && !this.registeredMethods.head) {
    /*
...
Route.prototype.dispatch = function dispatch(req, res, done) {
  ...
  */
  ...
  if (method === 'head' && !this.registeredMethods['head']) {
    method = 'get';
  }
  ...
  function nextHandler(happendedError) {
    ...
    // 如果是 all, 那么没有 method 这个值. 也就不会有 method 的判断了. 
    if (currentLayer.method && currentLayer.method !== method) {
      ...
      // Error 是具有透传性的. 如果一个中间价, 在调用 next 的时候, 不继续传递 error, 那么就认为这个中间件是将这个错误进行处理了. 
...
Route.prototype.all = function all() {
  ...
  */
  ...
  for (var i = 0; i < handles.length; i++) {
    ...
    this.registeredMethods._all = true; // 给 哈希 对象增加了一个 Bool 属性. 
...
// 在 Require 的时候, 就是在运行被 require 文件的代码. 所以这里其实就是调用. 
/*
...
可以像以上的方法增加中间件, 所以这里 handles 是一个循环遍历. 
*/
methods.forEach(function (method) {

  // 给 prototype 增加属性, 就是给类型增加实例方法. 
  Route.prototype[method] = function () {
    ...
    // 这是一个 For 循环, 在这里增加很多的 layer
    for (var i = 0; i < handles.length; i++) {
      var handle = handles[i];

      // 如果传递过来的不是 Function, 就抛出一个异常. 防卫式的写法, 确保传递过来的必须是一个函数对象. 
      if (typeof handle !== 'function') {
        var type = toString.call(handle);
        var msg = 'Route.' + method + '() requires a callback function but got a ' + type
        throw new Error(msg); // 这里的 error 没有地方进行捕获. 这是正确的, 因为这是在开发阶段, 早点的报错能够让开发人员更早的获取到出错信息. 
      }

      var layer = Layer('/', {}, handle);
      layer.method = method;

      this.registeredMethods[method] = true; // 标志了, 自己这个 route 可以使用 get, post 这个来处理. 
      this.stack.push(layer);
    }
